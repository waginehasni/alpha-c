...
void stage0(SDL_Surface *ecran,int *continuer,Mix_Music *intro,Mix_Chunk *effet,Mix_Chunk *effet2,Mix_Music *musique,int *mute,SDL_Rect *posvolb,Mix_Chunk *sonspell)
{
	...
	int boucle=1,i=0,a=0,p=0,posans=460,k=-1,y=0,x=0,j=0,e=2,intspell=0,nbrspell=0;
	SDL_Event event;
	...
	{       
		...
		SDL_BlitSurface(bulle, &bullanim[j/100], ecran, &posbulle);
...
void stage1(SDL_Surface *ecran,int *continuer,Mix_Music *intro,Mix_Chunk *effet,Mix_Chunk *effet2,Mix_Music *musique,int *mute,SDL_Rect *posvolb,Mix_Chunk *sonspell,Mix_Chunk *dying,Mix_Chunk *hit)
{
	...
	*idlel=NULL,*idler=NULL,*terre1=NULL,*terre2=NULL,*terre3=NULL,*jumpr=NULL,*jumpl=NULL,*surfacesecondes=NULL,*surfaceminutes=NULL
	...
	SDL_Rect posback,poshero,posmagicien,posmagicienactu,posspell,framed[8],frame2[8],pospolice,posgo,pose1,pose2,pose3,pose4,pose5,pose6,pose7,posenemie1,posenemie2,posenemie3,posenemie4,posenemie5,posenemie6,posenemie7,poshp,posrock1,posvie,posscore,poshpup,posminutes,possecondes,posnoir,
	...
	int tempsPrecedent = 0, tempsActuel = 0,tsec=0,tmin=0,theur=0;
	int y=0;
	int x=0;
	int j=0;
	int camera=0,menu=0;
	...
	enigmestage1=IMG_Load ("image/decoration/enigme.png");
	...
	while ((boucle)&&((*continuer)))
        ...
        scoreint=TTF_RenderText_Blended(fonts,scr, couleur);
       		...
       		switch(event.type)
        	{
				...
				break;
				/*case SDLK_u:
            			enigme(&ecran, &continuer, &intro, &effet, &effet2, &musique, &mute, &posvolb,&menu,&dying);*/
            		}
            		break;
		...
		if(keystates[SDLK_d])//right
		{
			        e=1;
            			...
            			if (coll==0)
            			{
            			scrolling(&poshero.x,&posmagicien.x,&posenigme1.x,&poshpup.x,&posback.x,&pospolice.x,&pose1.x,&pose2.x,&pose3.x,&pose4.x,&pose5.x,&pose6.x,&pose7.x,&posrock1.x,&postrap1.x,&posterre3.x,&posterre1.x,&posterre2.x,&posterre4.x,&posterre5.x,&posterre6.x,&posterre7.x,&posterre8.x,&posterre9.x,&camera,&ecran,&heroactu,e);
            			}
            			else
            			{
				//	camera+=speed;
		...
		if(keystates[SDLK_j]&&intspell==0&&(jump==0||CAR1==1||CAR2==1||CAR3==1||CAR4==1||CAR5==1||CAR6==1||CAR7==1||CAR8==1||CAR9==1||CAR10==1))//attack
		{
		        	...
		        	if(e==1)
		        	{
						...
						if(d1==1)
						{
		                		...
		                		Blit(enigmestage1,ecran,posenigme1.x,posenigme1.y);
		                		SDL_BlitSurface(health, NULL, ecran, &poshp);
		                		SDL_BlitSurface(score, NULL, ecran, &posscore);
		                		SDL_BlitSurface(vie, NULL, ecran, &posvie);
		                		SDL_BlitSurface(imgtime, NULL, ecran, &postime);
		...
		//début enigme
		if (poshero.x + poshero.w < posmagicien.x && poshero.x + poshero.w > posmagicien.x-200 && enig==0 )
		{
			enigme(&ecran, &continuer, &intro, &effet, &effet2, &musique, &mute, &posvolb,&menu,&dying);
			enig=1;
		}
